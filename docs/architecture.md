...
- **Widget surface:** template bodies are transparent and inherit the host page's background (Lotus renders widgets directly on the chat canvas), and every template sets `data-fit-content` so the resize bridge reports true content height instead of the descriptor's fallback. Read-only templates: `queue-progress`, `queue-items`, `queue-overview`, `item-carousel`, `item-inspector`, `tag-breakdown`, `due-forecast`, `session-summary`, `recent-items`, `streaks`, `atom-card`, `progress`. Interactive: `flashcard-drill`, `matching-pairs`, `drawing-canvas`, `markdown-canvas`, `coding-canvas`, `map-select`.
