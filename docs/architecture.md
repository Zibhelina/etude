...
- **Templates** (`applets/templates/`): self-contained HTML files with two injection points: `/*__THEME__*/` (CSS variables block) and `/*__DATA__*/` (JSON payload: atoms to drill, API base URL, queue id). The server renders `GET /applets/{template}?queue=Q&theme=T` by injecting both and a shared `ResizeObserver` bridge. The bridge posts `{lotus: 1, type: "resize", height}` to a supporting parent whenever the document height changes; other surfaces ignore it. This keeps every Etude applet free of nested scrollbars in Lotus without duplicating sizing code across templates. Templates never hardcode colors — only `var(--…)` from the theme contract.
