...
- **Templates** (`applets/templates/`): self-contained HTML files with two injection points: `/*__THEME__*/` (CSS variables block) and `/*__DATA__*/` (JSON payload: atoms to drill, API base URL, queue id). The server renders `GET /applets/{template}?queue=Q&theme=T` by injecting both. Templates never hardcode colors — only `var(--…)` from the theme contract.
...
- `meta.default_theme` names the active default; a request may override with `?theme=`. Agent soft-commands (defined in the skill, not in code): `#theme:everforest` (one-off), `#set-default-theme:everforest` (persists via `etude edit-meta default_theme=…`).
