...
- Templates in `widgets/templates/` include interactive flashcard-drill, matching-pairs, drawing-canvas, markdown-canvas, coding-canvas, and map-select plus read-only queue-progress, queue-items, queue-overview, item-carousel, item-inspector, tag-breakdown, due-forecast, session-summary, recent-items, streaks, atom-card, and progress. Matching-pairs reads `atom.widget_data.pairs`.
